import { FETCH_USER_AGENT, PREFERRED_RESPOSITORIES, GITHUB_BASE_URL, GITHUB_EMOJI_ISSUE_CLOSED, GITHUB_EMOJI_ISSUE_OPEN, GITHUB_EMOJI_PR_CLOSED, GITHUB_EMOJI_PR_DRAFT, GITHUB_EMOJI_PR_MERGED, GITHUB_EMOJI_PR_OPEN } from '../Constants';
import type { Issue, IssueState, PullRequest, PullRequestState, Query, Repository } from '../interfaces/octokit';
import { gql } from './gql';
...
export async function fuzzilySearchForRepository({ repository }: GhSearchRepositoriesParameters): Promise<APIApplicationCommandOptionChoice[]> {
	...
	if (isErr(result) || !result.value.search) {
		return PREFERRED_RESPOSITORIES;
	}
...
function getDataForIssue({ issue, ...repository }: Repository): IssueOrPrDetails {
	...
	return {
		...
		dateString,
		emoji: issue?.state === 'OPEN' ? GITHUB_EMOJI_ISSUE_OPEN : GITHUB_EMOJI_ISSUE_CLOSED,
		issueOrPr: 'ISSUE',
		number: issue?.number,
...
function getDataForPullRequest({ pullRequest, ...repository }: Repository): IssueOrPrDetails {
	...
	const getEmoji = () => {
		if (pullRequest?.state === 'CLOSED') return GITHUB_EMOJI_PR_CLOSED;

		if (pullRequest?.state === 'OPEN') {
			if (pullRequest?.isDraft) {
				return GITHUB_EMOJI_PR_DRAFT;
			}

			return GITHUB_EMOJI_PR_OPEN;
		}

		return GITHUB_EMOJI_PR_MERGED;
	};
